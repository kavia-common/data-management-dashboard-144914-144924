...
export default function Sessions() {
   ...
   *
   * Start/End date UI and related query/state have been removed per requirements.
   */
  ...
  // Remaining UI filters (date filters removed)
  ...
  // Keep URL query params in sync for dropdowns (so back/forward works). Remove date-related params.
  useEffect(() => {
    ...
    // Remove any legacy date params if present
    usp.delete("from");
    usp.delete("to");
    usp.delete("start");
    usp.delete("end");
  ...
  // Initialize dropdown selections from URL on first mount (date params ignored)
  useEffect(() => {
    const usp = new URLSearchParams(window.location.search);
  ...
  async function loadAggregates(qStr = "") {
     ...
     * Date range has been removed from this module.
     */
    setAggLoading(true);
    setAggError("");
    try {
      ...
      // Build dropdown options from aggregated dataset (all collected pages)
      const userPairs = all
        .map((it) => ({
          id: it?.user_id,
          name:
  ...
  async function load(page = 1, limit = meta.limit || 10, qStr = "", sortKey, sortDir) {
     ...
     * Date range parameters have been removed from requests.
     */
    ...
    try {
      ...
      // Note: Date range removed. Do not send start/end or from/to.
      if (filterTenantId && filterTenantId.trim()) {
        params.tenant_id = filterTenantId.trim();
      }
      if (filterUserName && filterUserName.trim()) {
        ...
        params.q = filterUserName.trim() + (qStr ? ` ${qStr}` : "");
      }
  ...
  useEffect(() => {
    ...
    const { key, dir } = lastSortRef.current || { key: "", dir: "asc" };
    load(1, meta.limit || 10, combinedQ, key, dir);
    // Charts also reflect the combined search above via loadAggregates in the debounced effect
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filterUserName, filterTenantId]);
